    ...
    return (
        ...
        >
            <CarouselContent className="-ml-4">
                {/* 
                   Ensure we have enough slides for smooth looping on all breakpoints.
                   We need at least 6-7 items to safely loop with 3 items visible.
                */}
                {(() => {
                    const REQUIRED_SLIDES = 6;
                    let displayStories = [...stories];
                    if (stories.length > 0 && stories.length < REQUIRED_SLIDES) {
                        const multiplier = Math.ceil(REQUIRED_SLIDES / stories.length);
                        displayStories = Array(multiplier).fill(stories).flat();
                    }

                    return displayStories.map((story, index) => (
                            ...
                            <div className="card-elegant overflow-hidden h-full flex flex-col group relative">
                                ...
                                <div className="relative h-64 overflow-hidden transform-gpu">
                                    <img
                                        ...
                                        onError={(e) => {
                                            e.currentTarget.src = 'https://images.unsplash.com/photo-1511632765486-a01980e01a18?q=80&w=2070&auto=format&fit=crop'; // Fallback wedding image
                                        }}
                                    />
                                    <div className="absolute inset-0 bg-gradient-to-t from-primary/80 via-transparent to-transparent opacity-80" />

                                    <div className="absolute bottom-4 left-4 right-4 z-10">
                                        <h3 className="font-serif text-2xl font-bold text-white mb-1">{story.names}</h3>
                                        <div className="flex items-center gap-2 text-amber-200 text-sm font-medium bg-black/40 w-fit px-2 py-1 rounded-full">
